...
public class DESAlgorithm {
    ...
    public byte[] crypt(byte[] message, byte[] key, String operation) {
        ...
        if (length == 8) return cryptText(message, key, operation);
    ...
    public byte[] cryptText(byte[] message, byte[] key, String operation) {
        ...
        /*
         * Разделение сообщения на две половины
         */
        ...
        for (int i = 0 ; i < noOfRounds ; i++) {
            ...
            /**
             * XOR для половины и раширенного ключа
             */
            byte[] roundKey = null;
    ...
    private byte[] sBox(byte[] input) {
        ...
        for (int i = 0; i < input.length ; i++) {
            byte block = input[i];
            /*
             * row - первый и последний биты
             * column - 4 бита посередине
             */
